...
The package is `flow:org.vibevm.zap/zap` version `1.0.0` under UPL-1.0. A
...
The normal user application route is `vibe install -g org.vibevm.zap/zap`,
which may resolve a small application-source proxy from an ordinary registry.
That proxy does not pretend to be this complete engine flow package.

For advanced project-flow development from a standalone Zap clone, select its
source registry explicitly:

```text
vibe build --help
vibe bin --help
vibe install flow:org.vibevm.zap/zap@=1.0.0 --registry <zap-checkout>/vibevm/vibepacks
vibe bin build zap --assume-yes --offline
vibe bin list --offline
vibe bin exec zap -- capabilities
```

An offline source build succeeds only when Zap and every
...
Version 1.1 adds a semantic map projection over an explicitly selected strategy
revision. Registered map queries use the existing machine Query request,
authenticated HTTP query interface and CLI store/request entrypoint. A map view
does not select a campaign, create work or introduce a new completion authority.

Cards retain underlying identities, structural work kinds, work purposes,
source state, acceptance meaning and typed relationships. Optional descriptive
work assessments keep effort, elapsed duration, waiting, complexity, difficulty
and uncertainty separate. Missing information and stale assessments remain
explicit. Route summaries distinguish known subtotals and precedence bounds
from a resource-feasible schedule.
