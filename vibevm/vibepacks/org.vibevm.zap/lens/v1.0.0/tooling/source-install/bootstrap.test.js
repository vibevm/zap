...
test("bootstrap preserves structured paths, becomes ready and keeps status read-only", async () => {
  ...
  try {
    ...
    assert.equal(planned.value.registryUrl, pathToFileURL(planned.value.localRegistryRoot).href);
    ...
    assert.equal(manifest.includes(planned.value.registryUrl), true);
    for (const input of [
    ...
    assert.equal(
      fixture.processCalls.every((call) => !call.args.includes("--no-default-registry")),
      true,
    );
    assert.equal(
      existsSync(join(planned.value.localRegistryRoot, "org.vibevm.zap", "lens", "v1.0.0")),
      true,
    );
    assert.equal(
      existsSync(join(planned.value.localRegistryRoot, "org.vibevm.zap", "zap", "v1.0.0")),
      true,
    );
    assert.deepEqual(
...
test("source install preserves a suspended verified binary generation cache", async () => {
  const fixture = createFixture();
  try {
    const host = join(fixture.settingsDir, "opt", "apps", "zap");
    const retained = join(host, "generations", "b".repeat(64), "payload", "bin", "zap.exe");
    mkdirSync(dirname(retained), { recursive: true });
    writeFileSync(retained, "retained verified binary cache\n");
    const result = await executeBootstrap(fixture.options("install"), fixture.ports);
    assert.equal(result.ok, true, result.message);
    assert.equal(existsSync(retained), true);
    assert.equal(
      fixture.processCalls.some((call) => call.operation === "install"),
      true,
    );
  } finally {
    fixture.close();
...
function createRegistryLensSource(registryDir) {
  const root = join(registryDir, "org.vibevm.zap", "lens", "v1.0.0");
  const files = [
    "LICENSE.md",
    "package-lock.json",
    "package.json",
  ...
  const engine = join(registryDir, "org.vibevm.zap", "zap", "v1.0.0");
  mkdirSync(join(engine, "crates", "zap-cli", "src"), { recursive: true });
  writeFileSync(
    join(engine, "vibe.toml"),
    '[package]\nname = "zap"\ngroup = "org.vibevm.zap"\nversion = "1.0.0"\n',
  );
  writeFileSync(join(engine, "Cargo.toml"), '[workspace]\nmembers = ["crates/zap-cli"]\n');
  writeFileSync(join(engine, "Cargo.lock"), "# fixture lock\n");
  writeFileSync(join(engine, "crates", "zap-cli", "src", "main.rs"), "fn main() {}\n");
}
