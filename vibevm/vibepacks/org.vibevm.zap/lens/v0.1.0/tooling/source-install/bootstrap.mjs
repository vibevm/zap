...
import { prepareLensSourceSnapshot } from "./bootstrap-snapshot.mjs";
import {
  directoryEntries,
  exists,
...
export async function resolveBootstrapPlan(options, ports) {
  ...
  const plan = {
    ...
    registryUrl: pathToFileURL(registryDir.value).href,
...
async function executeBootstrapInner(options, ports) {
  ...
  const snapshot = await prepareLensSourceSnapshot(planned.value, ports.fs);
  if (!snapshot.ok)
...
async function admitInstallRoot(plan, fs) {
  ...
  if (entries !== null && entries.length > 0) {
    ...
    return marker === null
      ? failure("refusing unrelated nonempty installation directory")
      : validateMarker(marker, plan.settingsDir);
  }
...
function vibeCommand(file, operation, root, offline, profile) {
  ...
  if (operation !== "undeploy")
    args.push("--no-default-registry", "--prefer-local", "--assume-yes");
