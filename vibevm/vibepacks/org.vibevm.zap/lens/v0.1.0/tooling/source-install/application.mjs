...
export async function runApplicationAdapter(environment = process.env, injected = {}) {
  ...
  try {
    ...
    return writeApplicationReply(
      replyPath,
      result(context, "ready", { runtime: "node", entry: management }, outcome.message),
    );
  } catch (error) {
    return writeFailed(
      replyPath,
...
export function parseApplicationContext(value) {
  ...
  if (
    ...
    packageIdentity.version !== "1.1.0"
  )
    failure("application package differs from org.vibevm.zap/zap@1.1.0");
  if (
    installerPackage.group !== "org.vibevm.zap" ||
    installerPackage.name !== "lens" ||
    installerPackage.version !== "0.1.0"
  )
    failure("installer package differs from org.vibevm.zap/lens@0.1.0");
  if (
...
function result(context, status, management, message) {
  return {
