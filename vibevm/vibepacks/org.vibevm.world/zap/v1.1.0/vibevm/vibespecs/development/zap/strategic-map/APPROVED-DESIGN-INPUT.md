...
## Owner discussion: understandable map objects and routes

The Owner's subsequent proposal emphasizes human understanding: estimate route
extent, distinguish hard from complex work, give every object a name and a
structured explanation, and classify objects and relationships by meaning.
Visual mappings should be decided later. Collection needed only for display
may follow algorithm development. This section records the design response;
it does not authorize implementation or change the published Rust package.

### Keep extent, complexity, difficulty and uncertainty separate

These should be independent dimensions, not successive levels on a
complex-to-hard scale:

| Dimension | Question answered | Proposed representation |
| --- | --- | --- |
| Remaining effort | How much work remains under the current scope? | Estimate and range with units; exclude completed/sunk work |
| Elapsed duration | How long until the result, given dependencies and capacity? | Separate estimate/range and scheduling assumptions; include applicable waiting |
| Structural complexity | How many interacting parts, constraints and boundaries must be coordinated? | Evidence about coupling, dependency depth and affected boundaries, plus a qualified assessment |
| Difficulty | How demanding is the solution for the intended executor with its available knowledge/capabilities? | Anchored qualitative grade, executor assumptions and reasons |
| Uncertainty | How well do we understand the work and its estimates? | Known gaps, estimate precision, provenance and explicit unknown values |

A large repetitive migration can be long but straightforward. A small-looking
concurrency problem can be difficult and poorly understood. Waiting for an
external decision can dominate calendar time while requiring little active
effort. Consequence severity is another existing dimension and must not be
reused as difficulty.

If a discrete difficulty or complexity grade is useful, start with a small
rubric such as low, medium and high, with **unassessed** separate from low.
Define anchors and require a short explanation. Difficulty is conditional on
the executor and available knowledge; structural complexity concerns the
problem's interactions. A single numeric score should not hide that distinction.
Do not make these new ratings automatic dispatch or approval rules without
separately validating that use.

The dark-forest and long-road ideas are possible client mappings. Long-road
distance should primarily mean remaining effort or explicitly selected elapsed
duration. Structural complexity is not simply length. Unknown information is
not automatically high difficulty; uncertainty needs a distinct representation.
The domain data should not contain forest, color or road-length categories.

### Reuse the current estimates without overstating coverage

[IncrementalCost](../../../../crates/zap-domain/src/economics/model/cost.rs)
already separates agent-hours, elapsed time and passive wait, with intervals,
precision, unknowns and evidence. A missing upper bound stays unknown, not zero.
[CostForecastRecord](../../../../crates/zap-domain/src/economics/records.rs)
retains baseline, completed effects, cumulative actual, remaining estimate and
relevant basis. These are economics/change-scope records; their presence does
not mean every map object already has a valid route-distance estimate.

Derive route measures from typed applicable data. Do not convert prose cost
descriptions into numbers by guesswork. Count shared work once by identity.
Agent-hours can aggregate over selected distinct work, while elapsed duration
depends on prerequisite and acceptance edges, parallelism, resource contention
and waiting. A simple sum or unconditional maximum is not a general schedule.
Alternative routes need separate estimates under comparable assumptions.

Every displayed estimate should retain its units, scope, baseline, applicable
team/executor assumptions, basis and freshness. Geometry is an approximate
presentation: a two-dimensional layout cannot in general preserve every graph
distance exactly. The inspectable estimate/range remains authoritative, and a
layout change must not alter the campaign. Stable spatial orientation should
be considered when estimates change rather than moving every town after each
observation.

### A common inspectable card

Every map node and relationship should have a bounded human-readable view.
For a node, the proposed common card is:

- Stable ID and semantic type, independent of the display name.
- Short name and a plain description of what it represents.
- Purpose: why it matters and which outcome or decision it supports.
- Expected result or use; for work/milestones, how completion is established.
- Current state, its reason, blockers and relevant relationships.
- Available estimates, their explanations, uncertainty and freshness.
- Source/evidence references and a way to inspect the underlying record.

Type-specific fields add detail without overloading that common structure:
a milestone explains its result and proof boundary; work explains the action
and contract; an information opportunity explains which question or decision
it could help; an execution resource explains capacity, access and occupancy.
A relationship explains why it exists and whether it constrains preparation,
acceptance, support, consumption or route selection.

Much of this already exists in WorkRecord, TaskContract, knowledge records,
forecasts and their references. Prefer assembling their views over duplicating
the authoritative text. Keep an optional display alias or generated summary
separate from the goal and acceptance contract. A convenient explanation cannot
change requirements. Show absent or stale information explicitly.

Clicking an object should retrieve the current bounded view, not trigger a
fresh model call or silently manufacture an explanation. Generate optional
summaries when relevant source content changes, retain their provenance and
mark them stale when appropriate. The existing exact-record read mechanism is
supporting infrastructure; a complete common card is still a proposed composed
view, not a capability already shipped for every object.

### Classify by meaning, then choose visual conventions

Keep structural kind, work purpose, state and assessment as separate fields.
Reuse existing WorkKind and WorkType distinctions. Potential information
subtypes include question, source and evidence; execution resource subtypes
describe the constrained capability or capacity. Choose additional milestone
subtypes only where a different semantic contract warrants them.

Relationship types should distinguish prerequisite-for-preparation,
prerequisite-for-acceptance, contributes-to, supports, verifies, derived-from,
consumes and alternative-to. A route is a selected or candidate path/subgraph
over such relationships, not another word for every edge. A single object or
edge can be difficult, uncertain or blocked without changing its semantic type.
The client can later choose standard appearances for these meanings.

### Development order

Establish the minimal meaning now: stable identity, type, name, purpose/result,
acceptance boundary, state reasons, relationships, and honest unknown values.
These support planning, recovery and review as well as human understanding.
Reuse estimates and knowledge already needed by economics and selection.

Defer display-only enrichment: extra narrative polish, assessments unused by
the algorithm, visual distance calibration, terrain/icon choices and layout.
Collect optional information lazily or after the relevant algorithm work;
missing decorative enrichment must not block execution. Additional semantic
assessments need an explicit benefit and cost, not a mandatory model call for
every object on every revision.

The inexpensive part is exposing and explaining existing data. Reliable new
difficulty judgments or complete route forecasts may require real analysis.
This ordering preserves the Owner's goal of a fully understandable map without
turning explanation into a second uncontrolled planning workload.
