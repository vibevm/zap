...
When the configured registry exposes the strategic-map queries, select an exact
strategy revision and use overview, object and route requests from the
[map usage guide](../../flows/zap/ZAP-STRATEGIC-MAP-GUIDE.md). Follow the typed
result's continuation; an empty filtered page can still have more memberships.
...
assessments use their ordinary DataProposal command with the current source
fingerprint and exact record CAS; they do not grant approval or alter Work.
Opening a card never requires a new semantic/model call. Optional human-facing
enrichment must not delay required algorithm work.
