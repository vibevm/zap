...
This non-normative guide describes the [zap_domain public modules](../../../../crates/zap-domain/src/lib.rs). Its catalogs cover all 454 externally reachable local types across charter/intent/outcome, work and obligations, knowledge and proof, adaptive review, economics, acceptance, Owner control, strategic lowering, detached returns, Dreamer and viewer operations.

Paths in the catalogs are relative to `zap_domain`, so `intent::CharterRecord` means the public module path rather than a private implementation module. The inventory distinguishes 437 literal declarations and 67 macro emissions: 454 unique public types and 50 private-module implementation declarations. There are no public unions or foreign type reexports. Offline lowering types also have public `lowering::offline` aliases; each unique type is cataloged once at its shorter public path.
...
The catalogs map every unique public type once. The following module counts total 454; additional offline aliases do not represent additional types:
