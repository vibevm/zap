...
согласует результат и границы самостоятельности, затем агент уточняет план
и работает до принятого результата или заранее согласованного условия остановки.
...
[Полная карта](vibevm/vibespecs/research/zap/IDEA-MAP.xml) содержит 20 идей
и 28 связей. Та же карта есть в
...
```mermaid
flowchart TD
    O[Замысел и договор Владельца] --> G[Обязательства и карта неизвестного]
    G --> F[Факты и доказательства]
    F --> D[Решения с основаниями]
    D --> L[Уточнение плана и lowering]
    L --> W[Разрешённая работа]
    W --> F
    W --> A[Проверка и приёмка]
    O --> S[Вычисление условий остановки]
    F --> S
    S --> W
    D --> E[Журнал событий]
    L --> E
    A --> E
    E --> V[Детерминированные проекции и будущий viewer]
```

Та же процедура применяется к кампании, направлению и сложной задаче.
Lowering сохраняет каждое обязательство, его реализацию и способ проверки.
